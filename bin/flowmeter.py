...
class FlowMeter():
  ...
  pulses_per_second_per_liter = 7

  def __init__(self, displayFormat, pulses_per_second_per_liter):
    ...
    self.totalPour = 0.0
    self.enabled = True
    self.pulses_per_second_per_liter = pulses_per_second_per_liter
  ...
  def update(self, currentTime):
    ...
    if (self.enabled == True and self.clickDelta < 1000):
      ...
      self.flow = self.hertz / (FlowMeter.SECONDS_IN_A_MINUTE * self.pulses_per_second_per_liter)  # In Liters per second
